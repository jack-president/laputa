...
<c:url value="/rbac/sysrole/updateHaveOwnSysPermission" var="updateHaveOwnSysPermissionUrl"/>
<c:url value="/rbac/sysrole/readHaveOwnSysPermission/{0}" var="readHaveOwnSysPermissionUrl"/>
<c:url value="/rbac/sysrole/readNotHaveOwnSysPermission/{0}" var="readNotHaveOwnSysPermissionUrl"/>
...
<div>
    ...
    <div id="buttonDiv" align="center" hidden="hidden">
        ...
        <button id="submitButton" class="k-primary">下方列表为角色拥有权限</button>
    </div>
...
<script>
    laputaKendoRequire(["kendo/js/kendo.button", "kendo/js/kendo.grid"], function () {
        ...
        var leftSysPermissionFilterDataSource = makeDataSource(kendo.format("${readNotHaveOwnSysPermissionUrl}", ${sysRoleId}));
        var rightSysPermissionFilterDataSource = makeDataSource(kendo.format("${readHaveOwnSysPermissionUrl}", ${sysRoleId}));
        ...
        var initFun = function () {
            ...
            submitButton.bind("click", function () {
                ...
                var data = {
                    id: ${sysRoleId},
                    ownSysPermissionCollection: []
                };

                var selectItem = rightGrid.dataItems();
                if (selectItem != null && selectItem.length > 0) {
                    for (var i = 0; i < selectItem.length; ++i) {
                        if ($.inArray(data.ownSysPermissionCollection, selectItem[i].id) < 0) {
                            data.ownSysPermissionCollection.push(selectItem[i].id);
                        }
                    }
                }

                $.ajax({
                        url: "${updateHaveOwnSysPermissionUrl}",
                        type: 'POST',
                    ...
                    try {
                        window.parent.configSysRoleGrid.dataSource.read();
                        window.parent.configsysroleframe.close();
                    } catch (e) {
                        console.log(e);
                    }
                });
